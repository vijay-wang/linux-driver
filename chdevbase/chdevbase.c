...
static ssize_t read_chdevbase(struct file *filp, char __user *buf,
		size_t cnt, loff_t *offt)
{
	if(copy_to_user(buf, kernel_data, sizeof(kernel_data))) {
		...
		printk(KERN_INFO "chdevbase read %u byte(s) : %s\r\n", sizeof(kernel_data), kernel_data);

	return sizeof(kernel_data);
}

static ssize_t write_chdevbase(struct file *filp, const char __user *buf,
		size_t cnt, loff_t *offt)
{
...
static struct file_operations chdevbase_fops = {
	.owner = THIS_MODULE,
	.open = open_chdevbase,
